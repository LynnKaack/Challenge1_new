...
# so we check for if the file exists
if(!file.exists("crime_log.csv") && file.name.blotter=="crime-base.csv"){
  write.csv(x = data.frame("inserted"=0, "invalid"=0, "corrected"=0, "patched"=0, "not.patched"=0), 
            "crime_log.csv", row.names = F)
}
...
filter.data <- function(input.blotter){
  ...
  input.blotter <- input.blotter[!is.na(input.blotter$ZONE),]
  
  # Excluding the data with no neighborhood
  input.blotter <- input.blotter[!is.na(input.blotter$NEIGHBORHOOD),]
  
  # Excluding the data with no arrest_time
  input.blotter <- input.blotter[!is.na(input.blotter$ARREST_TIME),]
...
log.frame <- read.csv("crime_log.csv") # reading in the log file
