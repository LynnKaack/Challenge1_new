...
sink("All_Output_Lynn.log", type=c("output", "message"), append=TRUE, split=TRUE)


# Reading in the data from the shell
# It is important that it uses the first column as the rownames and does not 
# create an extra column. This is taken care of in Filter_DATA_Lynn.R by not writing the rownames.
...
replace.neighborhood.name <- function(input.name){
    if(!(input.name %in% accepted.hoods) && 
      ...
      if(length(replacement.hoods)>1){
        print(paste("There are multiple matching neighborhoods, which are", replacement.hoods))
      }
...
well.formatting <- function(data.input){
  ...
  for(i in 1:nrow(data.input)){
    ...
    if(!(data.input$NEIGHBORHOOD[i] %in% accepted.hoods)){
      print(i)
      data.input$NEIGHBORHOOD[i] <- replace.neighborhood.name(data.input$NEIGHBORHOOD[i])
...
# Report the rows that where skipped or corrected--------
  ...
  log.frame <- read.csv("crime_log_Lynn.csv")
